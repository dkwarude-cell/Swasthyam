...
function computeEffectiveCalories(grams, harmScore) {
  ...
  const multiplier = Math.min(1 + K_PENALTY * (hNorm ** 2), M_MAX);
  const effectiveKcal = rawKcal * multiplier;
  
  return {
    rawKcal: Math.round(rawKcal * 100) / 100,
    multiplier: Math.round(multiplier * 1000) / 1000,
    effectiveKcal: Math.round(effectiveKcal * 100) / 100
  };
}
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      for (const mealEntry of novemberMeals) {
        ...
        const calories = computeEffectiveCalories(oilAmount, harmScore);

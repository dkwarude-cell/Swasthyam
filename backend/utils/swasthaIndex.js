...
function computeDailyGoal(bmr, activityFactor, sRoll, hRoll) {
  const { R_FAT, R_VISIBLE, ALPHA_S, ALPHA_H, V_MIN, V_MAX, HA_MIN, HA_MAX } = CONSTANTS;
  
  // 1. Compute TDEE
  const tdee = bmr * activityFactor;
  
  // 2. Base oil calories (6.25% of TDEE typically)
  const vBase = R_FAT * R_VISIBLE * tdee;
  
  // 3. Harm adjustment factor
  const ha = clamp((100 - hRoll) / 100, HA_MIN, HA_MAX);
  
  // 4. Adjusted oil calories based on diet quality and oil health
 ...
 * @param {number} harmScore - Harm score of oil (0-100)
 * @returns {number} Multiplier M(h) in range [1.0, M_MAX]
 */
function computeMultiplier(harmScore) {
  const { K_PENALTY, M_MAX } = CONSTANTS;
 ...
 * @param {number} harmScore - Harm score of the oil (0-100)
...
function computeEffectiveCalories(grams, harmScore) {
  ...
  return {
    rawKcal: round(rawKcal, 2),
    multiplier: round(multiplier, 3),
    effectiveKcal: round(effectiveKcal, 2)
  };
}

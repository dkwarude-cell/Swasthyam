...
// Calculate SwasthaIndex values
function calculateSwasthaIndex(oilAmount, harmScore) {
  const rawKcal = oilAmount * 9; // 9 kcal per gram of oil
  
  // Compute multiplier based on harm score
  let multiplier = 1.0;
  if (harmScore >= 0 && harmScore <= 20) {
    multiplier = 1.0;
  } else if (harmScore > 20 && harmScore <= 40) {
    multiplier = 1.0 + ((harmScore - 20) / 20) * 0.15;
  } else if (harmScore > 40 && harmScore <= 60) {
    multiplier = 1.15 + ((harmScore - 40) / 20) * 0.15;
  } else if (harmScore > 60 && harmScore <= 80) {
    multiplier = 1.30 + ((harmScore - 60) / 20) * 0.1;
  } else {
    multiplier = 1.40 + ((harmScore - 80) / 20) * 0.1;
  }
  
  const effectiveKcal = rawKcal * multiplier;
  
  return { rawKcal, multiplier, effectiveKcal };
}
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        const { rawKcal, multiplier, effectiveKcal } = calculateSwasthaIndex(newOilAmount, record.harmScore);
